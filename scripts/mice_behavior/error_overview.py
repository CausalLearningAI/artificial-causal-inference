"""
Visual error overview for the mouse pairwise behavior classifier: for each
behavior (nt, nn), shows k=10 random example frames from each confusion-matrix
bucket (TP/TN/FP/FN, argmax-decision-rule), with the bucket's share of the
full validation set annotated next to its row label.

"Predicted" class = argmax over the 3-way softmax (none/nt/nn) — the natural
hard-decision rule, same one val_acc already uses during training.

Usage:
    python scripts/mice_behavior/error_overview.py [--variant cls|patchgrid]
"""
import argparse
import json
import random
...
from src.mice_behavior.report import collect_val_predictions_fast
...
K = 10
BUCKETS = ['TP', 'TN', 'FP', 'FN']
BEHAVIORS = [(1, 'nt', 'Nose-Tail Sniffing'), (2, 'nn', 'Nose-Nose Sniffing')]
...
def main():
    ...
    parser.add_argument('--variant', choices=['cls', 'patchgrid'], default='cls')
    args = parser.parse_args()
    use_patch_grid = args.variant == 'patchgrid'
    ...
    model = MouseBehaviorClassifier(
        ...
        use_patch_grid=use_patch_grid, dropout=cfg.get('dropout', 0.1),
    ).to(dev)
    ...
    val_data = FastBatchData(
        ...
        n_patches=16 if use_patch_grid else None, stride=cfg.get('stride', 1),
    )
    probs, labels = collect_val_predictions_fast(model, val_data, dev)
    pred_class = probs.argmax(axis=1)
    gi = val_data.gi
    a1, a2 = val_data.a1, val_data.a2
    n_total = len(labels)

    frame_paths = pd.read_csv(annotations_csv, usecols=['frame_path'])['frame_path'].values

    # Row plan: a title row + one row per bucket for each behavior, with a blank spacer row
    # between behaviors (not after the last one) for visual separation.
    ...
    for r, (kind, behavior_c, payload) in enumerate(row_plan):
        ...
        bucket = payload
        is_true = labels == behavior_c
        is_pred = pred_class == behavior_c
        mask = {
        ...
        for col in range(K):
            ...
            if col < len(chosen):
                ...
                ax.set_title(f'm{a1[i]}→m{a2[i]}  p={probs[i, behavior_c]:.2f}', fontsize=7)
            if col == 0:
                ax.text(-0.15, 0.5, row_label, transform=ax.transAxes, fontsize=9, fontweight='bold',
                        ha='right', va='center', rotation=0)

    fig.suptitle(f'{args.variant}: error overview (argmax decision rule, k={K} random examples per bucket)', fontsize=14)
    # tight_layout() doesn't handle the merged (full-row) title axes well — produces wildly
    # uneven gaps. GridSpec's own hspace/wspace already controls spacing, so just set margins.

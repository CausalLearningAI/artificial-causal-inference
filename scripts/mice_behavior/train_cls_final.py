...
apples-to-apples comparison against the patch-grid attention-pooling variant.

Produces results/mice_behavior/cls_report.png:
...
Also saves cls_history.json and cls_roc_pr_data.npz.
...
def main():
    ...
    with open(RESULTS_DIR / 'cls_history.json', 'w') as f:
        json.dump(history, f, indent=2)
    with open(RESULTS_DIR / 'cls_config.json', 'w') as f:
    ...
    fig.suptitle(f'CLS-token (baseline) mouse behavior classifier — {CFG}')
    fig.tight_layout()
    fig.savefig(RESULTS_DIR / 'cls_report.png', dpi=150)
    np.savez(RESULTS_DIR / 'cls_roc_pr_data.npz', **save_data)
    print(f'Saved {RESULTS_DIR / "cls_report.png"}')

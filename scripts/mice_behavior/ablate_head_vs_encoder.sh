...
# RESULT -- all six arms landed 2026-08-15, macro AP @ tol0 on the standing 24-observation val set
# ==============================================================================================
#     frozen_ctrl_s1                     0.4200     seed 1  \  noise floor: 0.0089 between seeds
#     frozen_ctrl_s42                    0.4289     seed 42 /  of one identical config
#
#     head_multiquery  (q4)              0.4262     -0.0027 vs ctrl_s42   FLAT (below control)
#     head_selfattn    (sa128)           0.4431     +0.0142 vs ctrl_s42   ~1.6x noise: marginal
#
#     ft_bitfit        (elr 1e-5)        0.4509     +0.0220 vs ctrl_s42   ~2.5x noise
#     ft_bitfit_hi     (elr 1e-3)        0.4902     +0.0613 vs ctrl_s42   ~6.9x noise   WINS
#
#     res448_k2_ft2_d4photo              0.4889     full fine-tune, 14,180,352 params
#     res448_k2_ft6_d4                   0.5243     6 blocks (best single model to date)
#
# Reading it against the decision table above: ft_bitfit wins, head arms flat -> "domain shift;
# cheap recalibration suffices -> run ssl_dapt".
#
# The headline is the bitfit_hi vs ft2 comparison, which was not one of the pre-registered
# contrasts and is the most informative number here: 0.4902 with 24,576 params vs 0.4889 with
# 14,180,352. A 577x smaller update, none of which touches a weight MATRIX, fully reproduces
# supervised fine-tuning. Whatever fine-tuning bought, it was reachable by rescaling and
# reshifting features the frozen weights already computed -- it was not new computation. The
# architectural story this ablation was built to test is therefore wrong as stated.
#
# The encoder LR mattered more than the arm did: bitfit at 1e-5 is +0.0220 and at 1e-3 is
# +0.0613. Running only the inherited 1e-5 would have produced a weak, easily-dismissed effect
# and quite possibly the opposite conclusion. That is exactly why --encoder-lr's help text says
# a bitfit null at 1e-5 alone would be uninterpretable.
#
# Caveats that survive: n=24 val observations, so per-observation Pearson r (SE ~0.2) still
# cannot rank these and is not reported as a sort key. The noise floor is two seeds of ONE
# config, not per-arm replicates, so treat sub-0.02 gaps (i.e. head_selfattn) as unresolved
# rather than as small real effects.
#
# ft6 at 0.5243 continues ft2 0.4889 -> ft4 0.5212 -> ft6 0.5243 and is orthogonal to all of
# the above: it says more capacity still helps, not that the capacity is doing new computation.
# The obvious follow-up, untested, is bitfit at 6 blocks.
